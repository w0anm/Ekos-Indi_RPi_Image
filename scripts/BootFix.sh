...
if [ -d /ssd ] && [ -d /ssd/usr/lib/u-boot ] ; then
    mntWritable='/ssd/'
else
    echo "The partition 'writable' was not found in /ssd/writable.  Make sure you have mounted your USB mass storage device (ex: sudo mount /dev/sda2 /mnt/writable)."
    exit 1
fi
echo "Found writable partition at $mntWritable"

# Find the "system-boot" boot filesystem mount
if [ -d /ssd/boot/firmware ] && [ -e /ssd/boot/firmware/vmlinuz ]; then
    mntBoot='/ssd/boot/firmware'
else
    echo "The 'boot' partition was not found in /ssd/boot.  Make sure you have mounted your USB mass storage device (ex: sudo mount /dev/sda1 /mnt/boot)."
    exit 1
fi
